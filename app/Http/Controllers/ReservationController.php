...
class ReservationController extends Controller
{
   ...
   public function setup($gateway = 'PayPal_Express') {
    ...
    public function index () {
        ...
        $data['specialAvls'] = SpecialAvailability::all();
    ...
    public function indexAdmin () {
        ...
        if(count($data['timetables']) > 0){
          return view('vendor.adminlte.bookingAdmin', $data);
        } else{
    ...
    public function change_date (Request $request) {
        $functions = TimeTables::with(['reservations'])->with(['availabilitys'])->get();
        
        /*foreach($functions->reservations as $reservation)
        {
            foreach(SpecialAvailability::where('function_id', $reservation->function_id) as $specialAvl)
            {
                if($specialAvl->date == $reservation->date)
                {
                    $reservation->availability = $specialAvl->availability;
                }
            }
        }*/
    ...
    public function create_bookingAdmin (Request $request) {
        $reserv = new Reservations;
        
        $reserv->function_id = $request->function;
        ...
        $countResv = 0;
        foreach(TimeTables::with(['reservations'])->get() as $reservation)
        {
            if($reserv->date == $reservation->date)
            {
                $countResv = $countResv + $reservation->number_total;
            }
        }
        
        $availability = $request->functionAvailability;
        
        foreach(SpecialAvailability::where('function_id', $reserv->function_id) as $specialAvl)
        {
            if($specialAvl->date == $reservation->date)
            {
        ...
        if(( $availability - $countResv -
            (((int) $request->adult) + ((int) $request->children))) < 0)
        {
            return redirect()->back()->with('error', 'No Availability');
        }
        
        $reserv->number_people_adult = (int) $request->adult;
        $reserv->number_people_children = (int) $request->children;
        $reserv->number_total = ((int) $request->adult) + ((int) $request->children);
        $reserv->full_name_user = $request->name;
        $reserv->price_total = 0;
        
        $reserv->save();
        
        return redirect('reservations')->with('success', 'Reservation Was Successfully Created!');
    }
