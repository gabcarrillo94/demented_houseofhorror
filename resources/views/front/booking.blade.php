     ...
     <table class="table table-list-search">
                  ...
                  <tbody class="body_table">
                      @foreach($functions as $function)
                        ...
                        @foreach($specialAvls as $specialAvl)
                            @if($specialAvl->date==$reservation->date && $specialAvl->function_id==$function->id)
                                <?php $availability = $specialAvl->availability; ?>
                            @endif
                        @endforeach
                          ...
                          <tr>
                              ...
                              <td class="availability"> <?php echo ($availability - $total<0)?'<span style="color:red"> Sold Out </span>':$availability - $total ?> </td>
                              <td>
                                  <button  type="button" data-toggle="modal" data-target="#book_now" data-price="{{ @$function->price }}" data-availability="<?= $availability - $total ?>" data-id="{{@$function->id}}" class="btn btn-success book-now"> Book now </button>
                              </td>
...
$('.date_booking').datepicker({ 'format': 'mm-dd-yyyy'});
  ...
  $('.date_booking').change(function (e){
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for(var i=0; i < parsed.length; i++){
              ...
              for(var y=0; y < parsed[i].availabilitys.length; y++)
              {
                if(date_booking.val() == parsed[i].availabilitys[y].date)
                {
                  parsed[i].availability = parsed[i].availabilitys[y].availability;
                }
              }
              
              console.log('sub_total' + sub_total);
              total = parsed[i].availability - sub_total;
    ...
    if(validate_fechaMayorQue(date_tomorrow, date_booking.val()))
    {
        ...
        $('.date-view').html('For same day booking please call for availability');
